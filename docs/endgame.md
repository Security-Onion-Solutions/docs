...
If Endgame support is enabled, then [Dashboards](dashboards.md) and [Hunt](hunt.md) will have an `Endgame` action on the Actions menu. Clicking that action will pivot to Endgame Console based on the `agent.id` field.