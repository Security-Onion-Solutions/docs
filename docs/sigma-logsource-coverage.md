...
Elastic Defend does not emit equivalent events for these categories. Deploying [Sysmon](sysmon.md) with an appropriate configuration provides the telemetry; without it, these rules will never fire. Guided Analysis Playbooks are not currently planned for these categories.
