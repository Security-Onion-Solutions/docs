...
To prevent tampering, our Docker images are signed using GPG keys. [Soup](soup.md) verifies GPG signatures any time Docker images are updated.
...
If a service is not writing its logs to `/opt/so/log`, then you may need to check the Docker logs for more detail. For example, to check the Docker logs for [Kibana](kibana.md):
