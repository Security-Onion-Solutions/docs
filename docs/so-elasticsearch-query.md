...
You can use `so-elasticsearch-query` to submit a cURL request to the local Security Onion [Elasticsearch](elasticsearch.md) host from the command line.
...
Here's a more complicated example that includes piping the output to [jq](jq.md):
